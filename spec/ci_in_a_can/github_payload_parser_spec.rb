...
describe CiInACan::GithubPayloadParser do
  ...
  [:branch, :ref, :compare, :sha, :git_ssh, :repo].to_objects {[
    ...
    describe "parse" do
      ...
      it "should convert the http to a git ssh" do
        payload = CiInACan::GithubPayloadParser.new.parse content
        payload[:git_ssh].must_equal test.git_ssh
      end

      it "should return the repo" do
        payload = CiInACan::GithubPayloadParser.new.parse content
        payload[:repo].must_equal test.repo
      end

      it "should stamp the sha" do
        payload = CiInACan::GithubPayloadParser.new.parse content
        payload[:sha].must_equal test.sha
      end

      it "should stamp the branch" do
        payload = CiInACan::GithubPayloadParser.new.parse content
        payload[:branch].must_equal test.branch
      end
